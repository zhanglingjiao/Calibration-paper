...
calib_q=function(ftrain, Betay, theta, cov, alpha, v, delta ,q, Vq){
  ...
  psi=N*cov%*%phi 
  psi_b=t(psi[1:nn,]) 
  psi_q=as.vector(psi_b%*%colMeans(ppxb))+pxb-q#n*1     
...
for (j in 1:mm){ 
  ...
  q1=q+delta
  q2=q-delta
  npara_q1=(mean(ind1*ind3)*(q1-h))/(h*mean(ind1*ind2))
  npara_q2=(mean(ind1*ind3)*(q2-h))/(h*mean(ind1*ind2))
  npara_q[j] <- (npara_q1-npara_q2)/(2*delta)
}
...
nphi_nm=phi_an-phi_am
nphi_nm[mm,]=psi_q

V=nphi_nm%*%t(nphi_nm)/N^2
stat=t(U)%*%solve(V)%*%U

pvalue=pchisq(stat, df=mm, lower.tail=FALSE)
return(list(stat=stat,U=U,Un=npara,Um=mpara,V=V,vtotal=vtotal,pvalue=pvalue,case_para=case_para,case_npara=case_npara))
}
